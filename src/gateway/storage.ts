import type { ChannelConfig, GatewayEnv, LineWebhookEvent } from "./types";
...
export async function recordEvent(env: GatewayEnv, channelKey: string, event: LineWebhookEvent): Promise<void> {
  ...
  await env.DB.prepare(
    ...
    .bind(channelKey, lineUserId, event.type ?? null, messageType, messageText, event.replyToken ?? null, event.timestamp ?? null, JSON.stringify(event))
    .run();
...
export async function tryApplyBindingCode(env: GatewayEnv, channelKey: string, lineUserId?: string, text?: string): Promise<void> {
