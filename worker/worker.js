...
async function serveAiWearSharePage(env, pathname, corsHeaders) {
  ...
  const html = `<!doctype html><html lang="zh-TW"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><meta name="description" content="${escapeHtml(description)}"><meta property="og:type" content="website"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:url" content="${escapeHtml(shareUrl)}"><meta property="og:image" content="${escapeHtml(imageUrl)}"><meta property="og:image:secure_url" content="${escapeHtml(imageUrl)}"><meta property="og:image:type" content="image/jpeg"><meta name="twitter:card" content="summary_large_image"><style>body{margin:0;font-family:Arial,'Noto Sans TC',sans-serif;background:#f4f7fb;color:#101828}.wrap{max-width:720px;margin:0 auto;padding:22px}.card{background:#fff;border:1px solid #d8e0eb;border-radius:18px;padding:18px;box-shadow:0 16px 42px rgba(16,24,40,.08)}.shareHeader{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.shareTitle{font-size:15px;font-weight:900;color:#101828}.shareTag{appearance:none;border:1px solid #b7f0c9;background:#eafff1;color:#058b3f;border-radius:999px;padding:8px 14px;font-size:15px;font-weight:900;cursor:pointer}.shareTag:disabled{opacity:.55;cursor:default}img{display:block;width:100%;border-radius:14px;border:1px solid #e4e9f2}h1{font-size:24px;margin:14px 0 8px}p{color:#526070;line-height:1.6}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.btn{display:inline-block;background:#06c755;color:#fff;text-decoration:none;font-weight:900;border-radius:12px;padding:12px 16px}.btn.secondary{background:#fff;color:#101828;border:1px solid #cfd8e6}.hint{font-size:14px;color:#667085;margin-top:10px}.shareStatus{font-size:13px;color:#14833b;margin:8px 0 0;min-height:18px}@media(max-width:480px){.wrap{padding:12px}.card{padding:14px;border-radius:16px}h1{font-size:22px}}</style><script src="https://static.line-scdn.net/liff/edge/2/sdk.js"></script></head><body><main class="wrap"><section class="card"><div class="shareHeader"><span class="shareTitle">AI 眼鏡試戴</span><button class="shareTag" id="shareTargetBtn" type="button">分享</button></div><img src="${escapeHtml(imageUrl)}" alt="AI 眼鏡試戴分享圖"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p><p class="shareStatus" id="shareStatus"></p><div class="actions"><a class="btn" href="${escapeHtml(lineAppUrl)}">用 LINE 開啟試戴</a><a class="btn secondary" href="${escapeHtml(browserUrl)}">瀏覽器版</a></div><p class="hint">從 FB 開啟時，請優先點「用 LINE 開啟試戴」，避免進入 LINE 帳密登入頁。</p></section></main><script>const LIFF_ID=${JSON.stringify(liffId)};const FLEX_MESSAGE=${JSON.stringify(flexPayload)};const LINE_SHARE_URL=${JSON.stringify(shareLineUrl)};let liffReady=false;function setShareStatus(text,isError){const el=document.getElementById('shareStatus');if(el){el.textContent=text||'';el.style.color=isError?'#b42318':'#14833b';}}async function openShareTargetPicker(){const btn=document.getElementById('shareTargetBtn');if(btn)btn.disabled=true;try{if(!window.liff||!liff.shareTargetPicker)throw new Error('LIFF shareTargetPicker unavailable');if(!liffReady){await liff.init({liffId:LIFF_ID});liffReady=true;}if(!liff.isLoggedIn()){liff.login({redirectUri:location.href});return;}await liff.shareTargetPicker([FLEX_MESSAGE]);setShareStatus('已開啟 LINE 分享。',false);}catch(err){console.warn('shareTargetPicker failed',err);setShareStatus('目前環境無法直接開啟 LINE 分享，已改用一般分享連結。',true);location.href=LINE_SHARE_URL;}finally{if(btn)btn.disabled=false;}}document.getElementById('shareTargetBtn')?.addEventListener('click',openShareTargetPicker);</script></body></html>`;
