...
function rewriteAiWearMobileShareHtml(html) {
  return String(html || "")
    ...
    .replace("const sharePayloads = {};\n    function esc", "const sharePayloads = {};\n    const AI_WEAR_ADD_FRIEND_URL = \"https://lin.ee/9rvLYhF\";\n    function esc")
    .replace("function forceAiWearLineRelogin(message){state.auth.idToken=\"\";state.auth.points=null;state.auth.pointError=\"LINE 登入逾時，正在重新登入\";renderMember();setStatus(\"LINE 登入已逾時，正在重新登入。\", false);try{if(window.liff&&liff.isLoggedIn())liff.logout();}catch(_err){}try{if(window.liff)liff.login({redirectUri:location.href});}catch(_err){location.reload();}}\n    function seriesIntro", "function forceAiWearLineRelogin(message){state.auth.idToken=\"\";state.auth.points=null;state.auth.pointError=\"LINE 登入逾時，正在重新登入\";renderMember();setStatus(\"LINE 登入已逾時，正在重新登入。\", false);try{if(window.liff&&liff.isLoggedIn())liff.logout();}catch(_err){}try{if(window.liff)liff.login({redirectUri:location.href});}catch(_err){location.reload();}}\n    function aiWearTryUrl(){const liffId=String(state.settings.liffId||\"2007221311-ISFxRBY3\").trim(); return liffId?`https://liff.line.me/${encodeURIComponent(liffId)}?aiWearTry=1`:(location.origin+\"/ai-wear\");}\n    function shouldGateAiWearTry(){const params=new URLSearchParams(location.search); return params.has(\"aiWearTry\")||sessionStorage.getItem(\"aiWearPendingTry\")===\"1\";}\n    function clearAiWearTryFlag(){try{sessionStorage.removeItem(\"aiWearPendingTry\");}catch(_err){} const url=new URL(location.href); const had=url.searchParams.has(\"aiWearTry\"); url.searchParams.delete(\"aiWearTry\"); if(had){history.replaceState(null,\"\",url.pathname+(url.searchParams.toString()?\"?\"+url.searchParams.toString():\"\")+url.hash);}}\n    async function ensureAiWearFriendBeforeTry(){if(!shouldGateAiWearTry())return; if(!window.liff||!liff.getFriendship)return; try{const friendship=await liff.getFriendship(); if(friendship&&friendship.friendFlag){clearAiWearTryFlag(); setStatus(\"已確認加入 LINE 好友，可以開始 AI 試戴。\", true); return;} try{sessionStorage.setItem(\"aiWearPendingTry\",\"1\");}catch(_err){} setStatus(\"請先加入 LINE 好友，加入後回到此頁即可開始 AI 試戴。\", true); if(liff.openWindow){liff.openWindow({url:AI_WEAR_ADD_FRIEND_URL,external:false});}else{location.href=AI_WEAR_ADD_FRIEND_URL;}}catch(err){console.warn(\"LINE friendship check failed\",err); setStatus(\"無法確認 LINE 好友狀態，請先加入好友後再試戴。\", false);}}\n    function seriesIntro")
    .replace("setStatus(state.models.length?\"請上傳自拍照並選擇眼鏡款式。\":\"素材庫尚無眼鏡款式，請先到後台上傳。\", state.models.length?null:false); await autoShareFromQuery();}", "setStatus(state.models.length?\"請上傳自拍照並選擇眼鏡款式。\":\"素材庫尚無眼鏡款式，請先到後台上傳。\", state.models.length?null:false); await ensureAiWearFriendBeforeTry(); await autoShareFromQuery();}")
    .replace("const tryUrl=liffId?`https://liff.line.me/${encodeURIComponent(liffId)}`:(payload.shareUrl||location.origin+\"/ai-wear\");", "const tryUrl=aiWearTryUrl();")
...
async function saveAiWearMemberSettings(env, body) {
  ...
  let memberRef = await resolveAiWearCrmMemberRef(env, lineUserId);
  const currentSettings = memberRef ? await loadAiWearMemberSettings(env, lineUserId) : aiWearMemberSettingsResponse(null, lineUserId);
  const hasPurchaseLineUrl = body && (Object.prototype.hasOwnProperty.call(body, "purchaseLineUrl") || Object.prototype.hasOwnProperty.call(body, "purchase_line_url"));
  const hasShareCaption = body && (Object.prototype.hasOwnProperty.call(body, "shareCaption") || Object.prototype.hasOwnProperty.call(body, "share_caption"));
  const purchaseLineUrl = hasPurchaseLineUrl
    ? normalizeAiWearPurchaseLineUrl(body.purchaseLineUrl ?? body.purchase_line_url, true)
    : currentSettings.purchaseLineUrl;
  const shareCaption = hasShareCaption
    ? normalizeAiWearShareCaption(body.shareCaption ?? body.share_caption)
    : currentSettings.shareCaption;
  if (!memberRef) memberRef = `aiwear:${shortHash(lineUserId)}`;
  const sourceJson = JSON.stringify({ LINE_user_id: lineUserId, LINE_display_name: stringValue(profile.displayName), pictureUrl: stringValue(profile.pictureUrl), source: "ai-wear" });
  await env.DB.prepare(`
...
async function serveAiWearSharePage(env, pathname, corsHeaders) {
  ...
  const flexFooterContents = [
    ...(contactLineUrl ? [greenFlexButton("我也要試戴", contactLineUrl)] : []),
